...
class ChooseRounds:
    ...
    def __init__(self):
        ...
        # list to set up round button. First item in each
...
class Play:
    ...
    def __init__(self, how_many):
        ...
        for item in range(0, 6):
            self.choice_button = Button(self.choice_frame,
                                        width=15, command=lambda i=item: self.to_compare(self.button_colour_list[i]))
    ...
    def to_compare(self, user_choice):
        ...
        to_remove = self.button_colour_list.index(user_choice)
        self.button_colour_list.pop(to_remove)

        # get computer choice and add to list for stats
        # when getting score, change it to an integer before
        # appending
        comp_choice = random.choice(self.button_colour_list)
        ...
        self.round_result_label.config(bg=round_results_bg,
    ...
    def new_round(self):
        ...
        # empty button ;ist so we can get new colours
        self.button_colour_list.clear()

        # get new colours for buttons
        self.button_colour_list = self.get_round_colours()

        # set button bg, fg and text
        count = 0
        for item in self.choice_button_ref:
            item['fg'] = self.button_colour_list[count][2]
            item['bg'] = self.button_colour_list[count][0]
            item['text'] = self.button_colour_list[count][0]
            item['state'] = NORMAL
